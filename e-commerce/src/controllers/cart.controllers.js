...
class CartControllers {
  ...
  async purchaseCart(req = request, res = response) {
    try {
      ...
      // Usar el método actualizado que genera el ticket y calcula el total
      const { total, ticket } = await cartServices.purchaseCart(cid, req.user.email);

      res.status(200).json({ status: "ok", total, ticket });
    } catch (error) {
      errorLog(error, req);
      res.status(500).json({ status: "Error", msg: "Error interno del servidor" });
    }
