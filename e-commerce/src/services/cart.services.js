...
class CartServices {
  ...
  async purchaseCart(cid, userEmail) {
    ...
    // Actualizamos el carrito solo una vez con los productos que no tenían stock suficiente
    await cartDao.update(cid, { products });

    // Generar ticket si el total es mayor a 0
    let ticket = null;
    if (total > 0 && userEmail) {
      ticket = await ticketService.createTicket(total, userEmail);
    }
